...
def extract_pc_data(dataset_path):
...
def main(args):
    ...
    raw_pc_sequence = extract_pc_data(args.dataset_path)

    data_list = []
    label = []
    
    print("Running " + args.model + " on " + str(len(raw_pc_sequence)) + \
            " Point Cloud Sequences")
    for idx in tqdm(range(len(raw_pc_sequence))):
        data = {
                'point': raw_pc_sequence[idx],
                'calib': calib
            }
